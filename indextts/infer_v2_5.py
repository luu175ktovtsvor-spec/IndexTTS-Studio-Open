...
class IndexTTS2:
    ...
    def _empty_device_cache(self):
        """Release the active accelerator cache without assuming CUDA."""
        try:
            if "cuda" in str(self.device) and torch.cuda.is_available():
                torch.cuda.empty_cache()
            elif "mps" in str(self.device) and hasattr(torch, "mps"):
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path, lang,
        ...
        sampling_rate = 22050
        speech_generation_kwargs = {
            "do_sample": do_sample,
            "num_beams": num_beams,
            "repetition_penalty": repetition_penalty,
        }
        if do_sample:
            speech_generation_kwargs.update(
                top_p=top_p,
                top_k=top_k,
                temperature=temperature,
            )
        if num_beams > 1:
            speech_generation_kwargs["length_penalty"] = length_penalty
...
class QwenEmotion:
    def __init__(self, model_dir):
        ...
        # Accelerate's automatic placement does not reliably select Apple MPS.
        # Keep CUDA's upstream placement and use CPU as a safe fallback.
